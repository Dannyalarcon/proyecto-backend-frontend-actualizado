...
if(isset($data->id_agenda)){
    ...
    if($check_post_stmt->rowCount() > 0){
        ...
        $delete_post = "UPDATE agenda SET asistencia='cancelado', estatus = '2' WHERE id_agenda=:post_id_agenda";

        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':post_id_agenda', $post_id,PDO::PARAM_INT);
        
        if($delete_post_stmt->execute()){
            $msg['message'] = 'cita cancelado exitosamente';
        }else{
            $msg['message'] = 'error no cancelo la cita intentalo nuevamente ';
        }
