...
if(isset($data->id_deuda)){
    ...
    if($check_post_stmt->rowCount() > 0){
        ...
        $delete_post = "UPDATE deudas SET estatus = '1' WHERE id_deuda=:post_id_deuda";

        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':post_id_deuda', $post_id,PDO::PARAM_INT);
        
        if($delete_post_stmt->execute()){
            $msg['message'] = 'deuda activada exitosamente';
        }else{
            $msg['message'] = 'error no activo intentalo nuevamente ';
        }
